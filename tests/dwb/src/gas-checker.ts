...
import type {GroupedGasLogs, TransferResult} from './snip';
import { entries } from "@blake.regalia/belt";
import { transfer } from "./snip";
import { SX_ANSI_GREEN, SX_ANSI_MAGENTA, SX_ANSI_RESET, SX_ANSI_UNDERLINE } from './helper';
...
export class GasChecker {
	...
	compare(h_local: GroupedGasLogs, xg_used: bigint) {
		...
		for(const [si_group, a_logs_local] of entries(h_local)) {
			...
			// offset
			let xg_previous = a_logs_local[0]?.gas;

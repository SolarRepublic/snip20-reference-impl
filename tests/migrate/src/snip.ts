...
import {entries, stringify_json, timeout} from '@blake.regalia/belt';
...
export async function transfer(
	...
	for(;;) {
		...
		if(xg_owner_loss !== xg_amount) {
			// retry
			if(Date.now() - xt_started < 15e3) {
				console.warn(`Discrepancy in expected balance. Node may be falling behind; retrying contract query`);
				await timeout(6e3);
				continue;
			}

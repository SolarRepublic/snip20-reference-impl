...
import { k_wallet_a, k_wallet_b, k_wallet_c, k_wallet_d, P_SECRET_LCD, SR_LOCAL_WASM } from './constants.js';
import { migrate_contract, preload_original_contract } from './contract.js';
import { bank, bank_send } from './cosmos.js';
import { ExternallyOwnedAccount } from './eoa.js';
import { Evaluator } from './evaluator.js';
import { H_FUNCTIONS, transfer_from } from './functions.js';
import { G_GLOBAL } from './global.js';
import { Parser } from './parser.js';
import { test_dwb } from './test-dwb.js';
...
async function evaluate(s_program, k_snip) {
    // parse program
...
// transfer method
const perform_transfer = async (k_wallet, xg_amount, k_recipient, ...a_args) => {
    // create sender eoa
...
const perform_batch_transfer_from = async (a_actions) => {
    // verbose
    const a_out = [];
...
async function validate_state(b_premigrate = false) {
    ...
    await Promise.all(a_eoas.map(k_eoa => kl_queries.use(async () => {
        // destructure eoa
        const { bank: xg_bank, balance: xg_balance, address: sa_owner, alias: s_alias, transfers: a_events, } = k_eoa;
        // resolve
            ...
            // allowances
            {
                ...
                for (const [a_allowances, h_allowances, si_other, si_which] of a_pairs) {
                    ...
                    for (const g_allowance of a_allowances) {
                        ...
                        const g_allowance_local = h_allowances[g_allowance[si_other]];
                        // not found
                        if (!g_allowance_local) {
                            throw Error(`No allowances ${si_which} found for ${k_eoa.label} locally`);
                        }
                        // destructure local
                        const { amount: xg_amount, expiration: n_expiration, } = g_allowance_local;
...
}
{
    ...
    const [sg_code_id, sb16_codehash, [, s_err] = []] = await secret_contract_upload_code(k_wallet_a, atu8_wasm, 30000000n);
    // upload failed
    if (!sg_code_id) {
        throw Error(s_err);
    }
    ...
    // fund all aliases
    await bank_send(a_eoas_genesis[0], 5000000n, a_eoas_aliased);
    // fund first 1024 numeric accounts
    await bank_send(a_eoas_genesis[1], 5000000n, a_eoas_numeric);
    ...
    if (!g_batch_xfer_1 || !g_batch_xfer_2 || !g_batch_xfer_3 || !g_batch_xfer_4 || !g_batch_xfer_5)
        throw Error(`Batch transfer test failed`);
    // re-validate
    await validate_state(false);

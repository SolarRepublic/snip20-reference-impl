...
import { SX_ANSI_GREEN, SX_ANSI_RED, SX_ANSI_MAGENTA, SX_ANSI_RESET, SX_ANSI_YELLOW, SX_ANSI_CYAN } from './helper.js';
const delta_color = (xg_delta, nl_pad = 0) => (bigint_abs(xg_delta) >= 1n
...
export class GasChecker {
    _h_baseline;
    _xg_used;
    constructor(_h_baseline, _xg_used) {
        this._h_baseline = _h_baseline;
        this._xg_used = _xg_used;
    }
    compare(h_local, xg_used) {
        const { _h_baseline, _xg_used } = this;
        console.log(`  ⚖️  Gas usage relative to baseline:  ${xg_used === _xg_used
            ? `${SX_ANSI_GREEN}0`
            : delta_color(xg_used - _xg_used)}${SX_ANSI_RESET}`);
        // each group
        for (const [si_group, a_logs_local] of entries(h_local)) {
            ...
            for (let i_log = 1; i_log < a_logs_local.length; i_log++) {
                // ref log
                const { index: i_local, gap: xg_gap_local, comment: s_comment_local, } = a_logs_local[i_log];
...
//# sourceMappingURL=gas-checker.js.map
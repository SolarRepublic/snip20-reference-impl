import { __UNDEFINED, bigint_abs, F_IDENTITY, is_bigint, is_number, is_undefined, keys, snake, text_to_base64, transform_values } from '@blake.regalia/belt';
...
import { bank } from './cosmos.js';
import { ExternallyOwnedAccount } from './eoa.js';
export function handler(s_args, f_handle, g_hooks) {
    return {
        params: s_args,
        handler: f_handle,
        before: g_hooks?.before,
    };
}
const H_TYPE_CASTERS = {
    string: F_IDENTITY,
...
export class Evaluator {
    _h_functions;
    _hm_pending = new Map();
    constructor(_h_functions) {
        this._h_functions = _h_functions;
    }
    async _flush() {
        const { _hm_pending } = this;
        // each pending order
        await Promise.all([..._hm_pending.entries()].map(async ([k_eoa, [a_execs, f_handler]]) => {
            const xg_limit = 100000n + (80000n * BigInt(a_execs.length));
            // sign transaction
            const [atu8_raw, si_txn] = await create_and_sign_tx_direct(k_eoa.wallet, a_execs.map(([atu8]) => atu8), xg_limit);
            // update bank from gas spent
            bank(k_eoa, -BigInt(k_eoa.wallet.fees(xg_limit)[0][0]));
            // broadcast
            const a_result = await broadcast_result(k_eoa.wallet, atu8_raw, si_txn, K_TEF_LOCAL);
            // forward to handler
    ...
    async evaluate(k_parser, k_snip) {
        ...
        for (const g_command of k_parser.program()) {
            ...
            // destructure
            const { value: g_statement, value: { method: si_method, sender: si_sender, args: a_args, fail: b_fail, error: s_expect, debug: b_debug, }, } = g_command;
            // lookup method
            ...
            await Promise.all(g_method.params.replace(/\s*=>.*$/, '').split(/\s*,\s*/g).filter(F_IDENTITY).map(async (s_param, i_param) => {
                ...
                try {
                    ...
                    // wtf
                    if (is_bigint(z_value) && bigint_abs(z_value) > 900000000000n) {
                        debugger;
                    }
                    // delete key if value is undefined
                    if (is_undefined(z_value))
                        delete g_args_raw[s_name];
                }
                catch (e_cast) {
            ...
            if (b_flushed || sa_previous !== sa_sender || b_fail || !this._hm_pending.size) {
                ...
                _hm_pending.set(k_eoa, [[[atu8_exec, atu8_nonce, g_args_sim, g_statement]], async (a6_response, a_execs) => {
                        ...
                        a_responses.forEach(([a_response], i_msg) => {
                            ...
                            // call handler
                            g_method_local.handler(k_eoa, g_args_app, g_answer[keys(g_answer)[0]], a6_response);
                        });
                    }]);
                // set previous sender

...
import { P_SECRET_LCD, k_wallet_a, X_GAS_PRICE, k_wallet_b, k_wallet_c, k_wallet_d, N_DECIMALS, SI_SECRET_CHAIN, P_SECRET_RPC } from './constants.js';
import { DwbValidator } from './dwb.js';
import { GasChecker } from './gas-checker.js';
import { transfer } from './snip.js';
export async function test_dwb(k_contract) {
    ...
    async function transfer_chain(sx_chain) {
    ...
    // }
    {
        ...
        for (let i_sim = 1; i_sim <= N_SIMULATIONS; i_sim++) {
            ...
            const [atu8_raw, si_txn] = await create_and_sign_tx_direct(k_wallet_b, [
                encodeGoogleProtobufAny(SI_MESSAGE_TYPE_COSMOS_BANK_MSG_SEND, encodeCosmosBankMsgSend(k_wallet_b.addr, k_wallet.addr, [[`${1000000n}`, 'uscrt']])),
            ], 50000n);
            // submit all in parallel
            const [g_result_transfer, [xc_send_gas, s_err_send_gas], [g_res_increase, , [xc_code, s_err] = []] = [], ,] = await Promise.all([
                // #ts-expect-error secret app
                transfer(k_dwbv, i_sim % 2 ? 1000000n : 2000000n, k_app_a, k_app_sim, k_checker),
            ...
            const g_result_xfer = g_result_transfer;
            if (!k_checker) {
        ...
        for (let i_sim = N_SIMULATIONS - 2; i_sim > 0; i_sim--) {
            ...
            // #ts-expect-error secret app
            const g_result = await transfer(k_dwbv, 1000000n, k_app_owner, k_app_recipient, k_checker, k_app_a);
            if (!k_checker) {
                k_checker = new GasChecker(g_result.tracking, g_result.gasUsed);
            }
...
//# sourceMappingURL=test-dwb.js.map
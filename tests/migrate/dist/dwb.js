...
const { expect } = chai;
import { DwbEntry } from './dwb-entry.js';
import { SX_ANSI_DIM_ON, SX_ANSI_RESET, fail } from './helper.js';
const R_ENTRY = /\s*DelayedWriteBufferEntry\(([^]*?)\)\s*,?/y;
export function parse_dwb_dump(sx_dump) {
    ...
    for (;;) {
        ...
        if (!m_entry)
            break;
        a_entries.push(bytes(parse_json(m_entry[1])));
    }
    return {
...
export class DwbValidator {
    ...
    async sync() {
        ...
        // parse
        const { empty_space_counter: sg_empty, entries: a_entries, } = parse_dwb_dump(g_dwb_res.dwb);
        ...
        this._n_empty = parseFloat(sg_empty);
    ...
    toString(b_show_delta) {
        ...
        const empty_row = (c) => `┃   ${SX_ANSI_DIM_ON}` + `...(empty x ${c})`.padEnd(78, ' ') + `${SX_ANSI_RESET}` + ' ┃';
        let i_index = 0;
        let c_empty = 0;
